...
//            NSLog(@"dict----res:%@, pingRes:%@",dict,self.pingResDic);
        ...
        _pingResultHandler(pingItem.status,pingSummary);
...
- (void)pingResultWithUCPing:(PhonePing *)ucPing pingResult:(PPingResModel *)pingRes pingStatus:(PhoneNetPingStatus)status
{
    ...
    _pingResultHandler(status,pingDetail);
}


-(void)pingResult:(NSString *)pingRes pingStatus:(PhoneNetPingStatus)status{
    _pingResultHandler(status,pingRes);
}
