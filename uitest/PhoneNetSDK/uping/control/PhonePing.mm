...
{
    NSLog(@"ping设置地址:%@",[NSThread currentThread]);
        ...
        if (remote_addr.sin_addr.s_addr == INADDR_NONE) {
            ...
            if (remoteHost == NULL || remoteHost->h_addr == NULL) {
                //dns解析失败
                log4cplus_warn("PhoneNetPing", "access %s DNS error, remove this ip..",[host UTF8String]);
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.delegate pingResult:[NSString stringWithFormat:@"access %s DNS error, remove this ip..",[host UTF8String]] pingStatus:PhoneNetPingStatusError];
                });
...
- (void)startPingHosts:(NSString *)host packetCount:(int)count
{
    ...
    if (self.host == NULL) {
        self.isStopPingThread = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate pingResult:@"There is no valid domain..." pingStatus:PhoneNetPingStatusFinished];
        });
...
- (void)sendAndrecevPingPacket
{
    ...
    do {
        ...
        if (sent < 0) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.delegate pingResult:[NSString stringWithFormat:@"ping %s , send icmp packet error..",[self.host UTF8String]] pingStatus:PhoneNetPingStatusError];
            });
...
//            NSLog(@"PhoneNetPing , ping %@ , receive icmp packet timeout..\n",self.host );
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.delegate pingResult:[NSString stringWithFormat:@"ping %s ,receive icmp packet error,bytesRead=0",[self.host UTF8String]] pingStatus:PhoneNetPingStatusError];
            });
