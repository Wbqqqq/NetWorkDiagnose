...
- (void)tracerouteWithUCTraceRoute:(PhoneTraceRoute *)ucTraceRoute tracertResult:(PTracerRouteResModel *)tracertRes
{
    ...
    if (tracertTimeoutRes.length > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            self.tracertResultHandler(tracertRes.status,tracertDetail);
        });
...
- (void)tracerouteFinishedWithUCTraceRoute:(NSString *)tracertDetail
{
    self.tracertResultHandler(Enum_Traceroute_Status_finish,tracertDetail);
}
